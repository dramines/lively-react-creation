...
const UpcomingEvent = ({ event }: { event: DashboardEvent }) => (
...
const Dashboard = () => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // Load artists
      const artistsData = await ArtistsService.getAllArtists();
      ...
      const now = new Date();
      // Ensure we're working with proper date objects for comparison
      const upcomingEventsCount = eventsData.filter((e: any) => {
      ...
      setStats([
        ...
        { icon: DollarSign, title: 'Total des dépenses', value: totalExpenses.toFixed(2), prefix: '€' },
        { icon: CheckSquare, title: 'Tâches en cours', value: tasksInProgressCount.toString() },
      ]);
      
      // Format upcoming events - handle empty or invalid date fields
      const formattedEvents = eventsData
        ...
        .slice(0, 3) // Show more upcoming events
        .map((e: any) => {
          const date = new Date(e.date);
          return {
            day: date.getDate().toString(),
            month: date.toLocaleDateString('fr-FR', { month: 'long' }),
            title: e.titre || e.title || 'Événement sans titre',
            location: e.lieu || e.location || 'Emplacement non spécifié',
            artists: e.artistes || []
          };
        });
      ...
      // First get all "en_cours" tasks
      const inProgressTasks = tasksData.filter((t: Tache) => t.statut === 'en_cours');
      
      // Then add high priority tasks that aren't completed
      const highPriorityTasks = tasksData.filter((t: Tache) => 
        t.priorite === 'haute' && t.statut !== 'terminé' && t.statut !== 'en_cours'
      );
      
      // Combine them with in-progress tasks having priority in the list
      const combinedTasks = [...inProgressTasks, ...highPriorityTasks].slice(0, 5);
      
      const formattedTasks = combinedTasks.map((t: Tache) => ({
        id: t.id,
        titre: t.titre,
      ...
      const calendarEvents = eventsData
        .filter((evt: any) => evt.date) // Filter out events without a date
        .map((evt: any) => ({
          id: evt.id,
          title: evt.titre || evt.title || 'Événement sans titre',
  ...
  const handleCompleteTask = async (taskId: string) => {
    ...
    try {
      ...
      // Find the original task to get all properties
      const user = JSON.parse(localStorage.getItem('user') || '{}');
      const tasksData = await TasksService.getAllTasks(user.id);
      const originalTask = tasksData.find((t: Tache) => t.id === taskId);
      
      if (!originalTask) return;
      
      const updatedTask = { 
        ...originalTask, 
        statut: originalTask.statut === 'terminé' ? 'à_faire' : 'terminé' 
      };
