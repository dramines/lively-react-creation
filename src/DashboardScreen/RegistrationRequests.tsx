...
const RegistrationRequests = () => {
  ...
  const [pendingSaisonId, setPendingSaisonId] = useState<string | null>(null);
  ...
  const openAcceptModal = (userId: string, saisonId: string, requestId: string, userName: string, seasonName: string) => {
    setModalAction('accept');
    setModalMessage(`Voulez-vous vraiment accepter la demande d'inscription de ${userName} pour la formation "${seasonName}" ?`);
    setPendingRequestId(requestId);
    setPendingUserId(userId);
    setPendingSaisonId(saisonId);
    setShowModal(true);
  };

  const openRejectModal = (requestId: string, userName: string, seasonName: string) => {
    setModalAction('reject');
    setModalMessage(`Voulez-vous vraiment refuser la demande d'inscription de ${userName} pour la formation "${seasonName}" ?`);
    setPendingRequestId(requestId);
    setShowModal(true);
  };
  ...
  const closeModal = () => {
    ...
    setPendingSaisonId(null);
  };

  const handleAcceptRequest = async () => {
    if (!pendingUserId || !pendingSaisonId || !pendingRequestId) return;
    ...
    try {
      ...
      showTemporarySuccess("La demande a été acceptée avec succès");
  ...
  const handleRejectRequest = async () => {
    ...
    try {
      const response = await fetch('https://plateform.draminesaid.com/app/request_users.php', {
        ...
        body: JSON.stringify({ id: pendingRequestId })
      });
      ...
      showTemporarySuccess("La demande a été rejetée avec succès");
  ...
  const handleRefresh = async () => {
    try {
      ...
      toast({
        title: "Actualisation",
        description: "Les demandes ont été actualisées avec succès",
                          ...
                          <div className="flex gap-2">
                            <Button
                              ...
                              onClick={() => openAcceptModal(
                                ...
                                request.id_saison, 
                                request.id,
                                `${request.prenom_client} ${request.nom_client}`,
                                getSeasonName(request.id_saison)
                              )}
