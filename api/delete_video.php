...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($input['key']) && isset($input['id_video'])) {
        $key = $input['key'];
        $id_video = $input['id_video'];

        if (strpos($key, '38457') === 0) {
            // Prepare the SQL statement to prevent SQL injection
            $stmt = $conn->prepare("DELETE FROM Videos WHERE id_video = ?");
            $stmt->bind_param("i", $id_video); // 'i' indicates the variable is an integer
