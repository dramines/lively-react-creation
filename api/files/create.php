...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES["file"]) && $_FILES["file"]["error"] == 0) {
        ...
        if (in_array($filetype, $allowed)) {
            // Create storedfiles directory if it doesn't exist
            $upload_dir = "../../storedfiles/";
            ...
            if (move_uploaded_file($_FILES["file"]["tmp_name"], $upload_path)) {
                ...
                try {
                    $file_url = "https://respizenmedical.com/vilartprod/storedfiles/" . $new_filename;
                    ...
                    if ($stmt->execute()) {
                        ...
                        if (!$lastId) {
                            $getIdQuery = "SELECT id FROM files WHERE name = :name AND url = :url ORDER BY created_at DESC LIMIT 1";
                            $getIdStmt = $db->prepare($getIdQuery);
                            $getIdStmt->bindParam(":name", $filename);
                            $getIdStmt->bindParam(":url", $file_url);
                            $getIdStmt->execute();
                            
                            if ($row = $getIdStmt->fetch(PDO::FETCH_ASSOC)) {
                                $lastId = $row['id'];
                            }
                        }
